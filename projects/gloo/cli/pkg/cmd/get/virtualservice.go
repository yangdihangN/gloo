...
func VirtualService(opts *options.Options) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_ = printers.PrintVirtualServices(virtualServices, opts.Top.Output, opts.Create.DryRun)
			return nil
		},
	}
...
func Routes(opts *options.Options) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_ = printers.PrintRoutes(vs.VirtualHost.Routes, opts.Top.Output, opts.Create.DryRun)
			return nil
		},
	}
	return cmd
