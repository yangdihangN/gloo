...
func SetRoutePerFilterConfig(out *envoyroute.Route, filterName string, protoext proto.Message) error {
	if out.PerFilterConfig == nil {
		out.PerFilterConfig = make(map[string]*_struct.Struct)
	}
...
func configureMultiDest(in []*v1.WeightedDestination, outAction *envoyroute.RouteAction, filterName string, perFilterConfig PerFilterConfigFunc) error {
	...
	for i := range in {
		if out.Clusters[i].PerFilterConfig == nil {
			out.Clusters[i].PerFilterConfig = make(map[string]*_struct.Struct)
		}
...
func configureSingleDest(in *v1.Destination, out map[string]*_struct.Struct, filterName string, perFilterConfig PerFilterConfigFunc) error {
...
func setConfig(out map[string]*_struct.Struct, filterName string, config proto.Message) error {
	if config == nil {
		return nil
	}
